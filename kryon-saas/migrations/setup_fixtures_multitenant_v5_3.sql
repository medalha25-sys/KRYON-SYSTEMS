-- ==============================================================================
-- KRYON SYSTEMS — SETUP DEFINITIVO DE FIXTURES MULTITENANT (V5.3 REVISADO)
...
BEGIN
    ...
    -- 2.1 Desvincular profiles cujo organization_id aponte para as organizações A/B (Elimina erro 23503)
    ...
    -- 2.4 Excluir as organizações de teste (Agora 100% desvinculadas e livres de dependência de FK)
    DELETE FROM public.organizations WHERE id IN (v_org_a_id, v_org_b_id) OR lower(slug) IN (lower(v_slug_a), lower(v_slug_b));

    -- -------------------------------------------------------------------------
    -- FASE 3: INSERÇÕES DETERMINÍSTICAS DAS FIXTURES (SCHEMA COMPATÍVEL V5.1/V5.3)
    -- -------------------------------------------------------------------------
    ...
    -- 3.3 Restabelecer vínculo em profiles.organization_id
    ...
    RAISE NOTICE '======================================================================';
    RAISE NOTICE 'SUCESSO: Setup de Fixtures A/B V5.3 concluído e auditado!';
