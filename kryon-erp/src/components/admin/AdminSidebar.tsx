  ...
  useEffect(() => {
    async function checkModules() {
      ...
      if (orgId) {
          ...
          setConcreteActive(!!sub)
      } else {
          setConcreteActive(false)
      }
    }
    checkModules()
  }, [supabase])

  const isConcretePath = pathname?.startsWith('/')

  const mainLinks: { href: string; label: string; icon: React.ReactNode }[] = []

  const concreteLinks = [
    { href: '/executivo', label: 'Estratégico', icon: <TrendingUp size={18} /> },
    { href: '/dashboard', label: 'Dashboard', icon: <LayoutDashboard size={18} /> },
    { href: '/clientes', label: 'Clientes', icon: <Users size={18} /> },
    { href: '/produtos', label: 'Produtos/Serviços', icon: <Construction size={18} /> },
    { href: '/orcamentos', label: 'Orçamentos', icon: <FileText size={18} /> },
    { href: '/entregas', label: 'Logística', icon: <Truck size={18} /> },
    { href: '/estoque', label: 'Estoque', icon: <Package size={18} /> },
    { href: '/financeiro/contas-receber', label: 'Financeiro', icon: <DollarSign size={18} /> },
    { href: '/fiscal/nfe', label: 'Fiscal (NF-e)', icon: <FileText size={18} /> },
  ]
    ...
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-[#0F172A] border-r border-slate-800 text-slate-300 flex flex-col h-full shadow-2xl transition-all duration-300 ease-in-out relative group/sidebar`}>
      ...
      <div className={`p-6 border-b border-slate-800 flex flex-col gap-1 overflow-hidden ${isCollapsed ? 'items-center px-2' : ''}`}>
        <h1 className={`text-xl font-bold text-white flex items-center gap-2 whitespace-nowrap`}>
            ...
            {!isCollapsed && "Concrete ERP"}
        </h1>
        {!isCollapsed && <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold whitespace-nowrap">Unidade de Controle Industrial</span>}
      </div>
      
      <nav className="flex-1 p-4 space-y-8 overflow-y-auto no-scrollbar">
        {/* Isolated ERP Section */}
        <div>
            {!isCollapsed && (
