-- ==============================================================================
-- KRYON SYSTEMS — SETUP DEFINITIVO DE FIXTURES MULTITENANT (100% DADOS)
-- BANCO: Sistemas Kryon (pgpobxvkojawrstadrel.supabase.co)
-- MÓDULO: Kryon Lava Rápido (SQL V5.1 Final Pós-Desacoplamento)
-- OBJETIVO: INSERIR FIXTURES DETERMINÍSTICAS COM SCHEMA TOTALMENTE AUDITADO
-- STATUS: AGUARDANDO AUTORIZAÇÃO EXPRESSA DO USUÁRIO (NÃO EXECUTAR AINDA)
-- ==============================================================================
...
BEGIN
    ...
    -- -------------------------------------------------------------------------
    -- FASE 2: VERIFICAÇÃO ANTI-COLISÃO (GARANTIR BANCO LIMPO E SEM DUPLICIDADES)
    -- -------------------------------------------------------------------------
    
    -- 2.1 Verificar Organizations (IDs ou Slugs)
    SELECT COUNT(*) INTO v_conflict_count 
    FROM public.organizations 
    WHERE id IN (v_org_a_id, v_org_b_id) OR lower(slug) IN (lower(v_slug_a), lower(v_slug_b));
    IF v_conflict_count > 0 THEN
        RAISE EXCEPTION 'CONFLITO DETECTADO: % organização(ões) com IDs ou Slugs de teste já existem no banco.', v_conflict_count;
    END IF;

    -- 2.2 Verificar Services
    SELECT COUNT(*) INTO v_conflict_count 
    FROM public.services 
    WHERE id IN (v_serv_a_id, v_serv_b_id);
    IF v_conflict_count > 0 THEN
        RAISE EXCEPTION 'CONFLITO DETECTADO: % serviço(s) de teste já existem no banco.', v_conflict_count;
    END IF;

    -- 2.3 Verificar Customers
    SELECT COUNT(*) INTO v_conflict_count 
    FROM public.customers 
    WHERE id IN (v_cust_a_id, v_cust_b_id);
    IF v_conflict_count > 0 THEN
        RAISE EXCEPTION 'CONFLITO DETECTADO: % cliente(s) de teste já existem no banco.', v_conflict_count;
    END IF;

    -- 2.4 Verificar Appointments
    SELECT COUNT(*) INTO v_conflict_count 
    FROM public.appointments 
    WHERE id IN (v_appt_a_id, v_appt_b_id);
    IF v_conflict_count > 0 THEN
        RAISE EXCEPTION 'CONFLITO DETECTADO: % agendamento(s) de teste já existem no banco.', v_conflict_count;
    END IF;

    -- -------------------------------------------------------------------------
    -- FASE 3: INSERÇÕES DETERMINÍSTICAS DAS FIXTURES (SCHEMA 100% AUDITADO)
    -- -------------------------------------------------------------------------

    -- 3.1 Organizações de Teste (O trigger on_organization_owner_created vinculará owner_id em organization_members e profiles.organization_id)
    ...
    -- 3.3 Clientes Distintos (Colunas auditadas e compatíveis: sem shop_id, com tenant_id)
    ...
    -- 3.4 Agendamentos Distintos (Colunas auditadas e compatíveis)
    ...
    -- -------------------------------------------------------------------------
    -- FASE 4: VALIDAÇÕES PÓS-SETUP E AUDITORIA DE INTEGRIDADE
    -- -------------------------------------------------------------------------

    -- 4.1 Validar exatamente 2 organizações criadas
    SELECT COUNT(*) INTO v_val_orgs 
    FROM public.organizations 
    WHERE id IN (v_org_a_id, v_org_b_id);
    IF v_val_orgs != 2 THEN
        RAISE EXCEPTION 'ERRO PÓS-SETUP: Esperava 2 organizações, foram encontradas %.', v_val_orgs;
    END IF;

    -- 4.2 Validar exatamente 2 membros criados pelo trigger (role owner)
    SELECT COUNT(*) INTO v_val_members 
    FROM public.organization_members 
    WHERE organization_id IN (v_org_a_id, v_org_b_id)
      AND user_id IN (v_user_a_id, v_user_b_id)
      AND role = 'owner';
    IF v_val_members != 2 THEN
        RAISE EXCEPTION 'ERRO PÓS-SETUP: Esperava 2 organization_members criados pelo trigger, foram encontrados %.', v_val_members;
    END IF;

    -- 4.3 Validar que profiles.organization_id foi atualizado corretamente pelo trigger
    ...
    IF v_prof_a_org != v_org_a_id OR v_prof_b_org != v_org_b_id THEN
        RAISE EXCEPTION 'ERRO PÓS-SETUP: profiles.organization_id não corresponde aos IDs das organizações criadas.';
    END IF;

    -- 4.4 Validar exatamente 2 serviços
    SELECT COUNT(*) INTO v_val_services 
    FROM public.services 
    WHERE id IN (v_serv_a_id, v_serv_b_id);
    ...
    SELECT COUNT(*) INTO v_val_customers 
    FROM public.customers 
    WHERE id IN (v_cust_a_id, v_cust_b_id);
    ...
    SELECT COUNT(*) INTO v_val_appointments 
    FROM public.appointments 
    WHERE id IN (v_appt_a_id, v_appt_b_id);
    ...
    RAISE NOTICE '======================================================================';
    RAISE NOTICE 'SUCESSO: Setup de Fixtures A/B concluído e auditado!';
