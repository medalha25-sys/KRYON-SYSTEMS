...
interface ProductActivityCardsProps {
  activities?: ProductActivityItem[]
  isLoading?: boolean
}

export function ProductActivityCards({
  activities = mockProductActivities,
  isLoading = false
}: ProductActivityCardsProps) {
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="p-5 rounded-3xl bg-[#0B0F19] border border-white/[0.08] animate-pulse space-y-4"
            >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-purple-400/90 font-medium">Módulo Integrado</span>
                      </div>
                    </div>
