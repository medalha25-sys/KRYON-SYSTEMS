-- ==============================================================================
-- KRYON SYSTEMS — SETUP DEFINITIVO DE FIXTURES MULTITENANT (V5.2 CORRIGIDO)
-- BANCO: Sistemas Kryon (pgpobxvkojawrstadrel.supabase.co)
-- DATA: 05/09/2026
-- CORREÇÃO: Desvinculação prévia de profiles.organization_id (Zero erro FK 23503)
...
BEGIN
    ...
    -- -------------------------------------------------------------------------
    -- FASE 2: LIMPEZA SEGURA E CIRÚRGICA DE FIXTURES PRÉVIAS (ORDEM CORRETA DE FK)
    -- -------------------------------------------------------------------------
    
    -- 2.1 Desvincular profiles das organizações de teste (Evita erro 23503 em profiles_organization_id_fkey)
    UPDATE public.profiles 
    SET organization_id = NULL 
    WHERE organization_id IN (v_org_a_id, v_org_b_id) OR id IN (v_user_a_id, v_user_b_id);

    -- 2.2 Excluir registros dependentes de agendamentos, clientes e serviços das fixtures
    ...
    -- 2.3 Excluir membros das organizações de teste
    DELETE FROM public.organization_members WHERE organization_id IN (v_org_a_id, v_org_b_id);

    -- 2.4 Excluir as organizações de teste (Agora 100% desvinculadas e livres de dependência)
    DELETE FROM public.organizations WHERE id IN (v_org_a_id, v_org_b_id) OR lower(slug) IN (lower(v_slug_a), lower(v_slug_b));

    -- -------------------------------------------------------------------------
    -- FASE 3: INSERÇÕES DETERMINÍSTICAS DAS FIXTURES (SCHEMA COMPATÍVEL V5.1/V5.2)
    -- -------------------------------------------------------------------------
    ...
    -- 3.2 Garantir organization_members (Garantia idempotente)
    ...
    -- 3.3 Reestabelecer vínculo em profiles.organization_id
    ...
    RAISE NOTICE '======================================================================';
    RAISE NOTICE 'SUCESSO: Setup de Fixtures A/B V5.2 concluído e auditado!';
