...
const Services = () => {
    return (
        <section className="section py-12 sm:py-16 md:py-24 bg-[#08080c] relative overflow-hidden" id="soluções">
            {/* Background Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl h-[300px] sm:h-[400px] bg-blue-600/[0.03] rounded-full blur-[100px] sm:blur-[140px] pointer-events-none" />
            ...
            <div className="container-custom relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
                    {services.map((service, index) => (
                        <motion.div
                            key={index}
                            className="card flex flex-col items-center text-center p-5 sm:p-8 hover:-translate-y-1 transition-all"
                            initial={{ opacity: 0, y: 20 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            transition={{ delay: index * 0.08 }}
                        >
                            <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center border mb-4 sm:mb-6 ${service.bgColor}`}>
                                {service.icon}
                            </div>
                            <h3 className="text-base sm:text-xl font-bold text-white mb-2 sm:mb-3 tracking-tight">
                                {service.title}
                            </h3>
